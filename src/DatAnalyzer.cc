...
void DatAnalyzer::Analyze(){
  ...
  **************************************/
  for(unsigned int i=0; i<NUM_CHANNELS; i++) {
    ...
    std::cout << "amp: " << amp << "idx_min:" << idx_min << std::endl;
    ...
    float Re_b, Re_slope;
    bool fittable = true;
    fittable *= idx_min < (int)(NUM_SAMPLES*0.9);//*0.8);
    ...
    // if( fittable  && !config->channels[i].algorithm.Contains("None")) {
      ...
      **************************************/
      if ( config->constant_fraction.size() ) {
        float start_level =  - 2 * baseline_RMS;//- 3 * baseline_RMS;
        ...
        for(auto f : config->constant_fraction) {
          ...
          if( config->channels[i].algorithm.Contains("IL")) {
            ...
            cout << Form("%g: %g %g %g %g %g %g", f, t1, v1, t2, v2, amp*f, out) << std::flush;//<< endl;
